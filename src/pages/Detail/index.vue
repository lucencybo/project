        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <!-- <router-link> 不能使用声明式导航，因为我们不是直接跳到添加成功页面的
                而是要先在详情页发请求给后台，后台返回成功数据后，在手动跳转到添加成功页面</router-link>-->
  ...
  methods: {
    ...
    async addShopCart() {
      ...
      try {
        ...
        alert('添加购物车成功，将自动跳转到成功的页面')

        //因为成功的页面需要商品的数量，所以用过路由传参传递
        //成功的页面还需要显示当前商品的详情，所以我们也得把商品的信息传递过去（存储方法 ：localstorage和sessionStorage)
        sessionStorage.setItem('SKUINFO_KEY',JSON.stringify(this.skuInfo))//不写的话就是object,object
