  ...
  methods: {
    ...
    async pay() {
      ...
      try {
        ...
        //2、弹出一个消息框去展示二位码图片
        this.$alert(`<img src="${imgUrl}" />`, "请使用微信扫描支付", {
          dangerouslyUseHTMLString: true,
          showClose: false, //右上角没有×
          ...
          beforeClose: (action, instance, done) => {
            //关闭之前回调，如果不写这个回调，那么无论点击什么按钮，消息盒子都会强制关闭
        ...
        }).then(()=>{}).catch(()=>{});//函数的返回值是promise 处理报错

        //3、弹出消息同时循环的给后台发请求，获取该订单的支付状态数据
        //根据返回来的支付状态去决定要不要跳转到支付成功页面
        if (!this.timer) {
          this.timer = setInterval(async () => {
            //每两秒发一次请求获取支付状态信息
            const result = this.$API.reqOrderStatus(this.payInfo.orderId);
            if (result.code === 200) {
              //支付成功
              //停止定时器  跳转到支付成功的页面 把当前的状态保存起来 以便用户点击我已成功支付的时候去判定
              this.status = 200;
              clearInterval(this.timer); // clearInterval清除定时器，停止给定编号的定时器，并没有清空存储编号的变量
              this.timer = null;
              //跳转过去之后手动关闭我们弹出信息框
